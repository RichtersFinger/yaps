class puzzlepiece {
	...
	setuptestclip() {
		...
		this.pathouter = [[this.i*unitx-0.01, this.j*unity-0.01], [(this.i + 1)*unitx+0.01, (this.j + 1)*unity+0.01]];
		// set correct camera space position in complete puzzle
		this.x = this.i*unitx*this.parentpuzzle.lx;
		this.y = this.j*unity*this.parentpuzzle.ly;
		// calculate the width and height of the piece in camera space
		// with margin (pathouter)
		this.w = (this.pathouter[1][0] - this.pathouter[0][0])*this.parentpuzzle.lx;
		this.h = (this.pathouter[1][1] - this.pathouter[0][1])*this.parentpuzzle.ly;
		// precise (pathinner)
		// this.w = unitx*this.parentpuzzle.lx;
		// this.h = unity*this.parentpuzzle.ly;
	}
	applyclip() {
		...
		if (this.pathinner.length > 0) {
			...
			this.divcontainer.style.width = (this.pathouter[1][0] - this.pathouter[0][0])*this.parentpuzzle.dimx + "px";
			this.divcontainer.style.height = (this.pathouter[1][1] - this.pathouter[0][1])*this.parentpuzzle.dimy + "px";
			...
			this.img.style.left = -this.pathouter[0][0]*this.parentpuzzle.dimx + "px";
			this.img.style.top = -this.pathouter[0][1]*this.parentpuzzle.dimy + "px";
		}
	}
}
