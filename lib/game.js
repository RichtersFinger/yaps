...
function setupSocketCommunication() {
  ...
  iosocket.on('startMovingPiece', ( i, j ) => {
    console.log('okay');
  ...
  iosocket.on('updatePieceCoordinates', ( i, j, x, y, z, angle ) => {
    ...
    thispuzzle.pieces[i][j].x = y;
    thispuzzle.pieces[i][j].x = z;
    thispuzzle.pieces[i][j].divcontainer.style.zIndex = thispuzzle.pieces[i][j].z;
        ...
        for (let j = 0; j < thispuzzle.layout[1]; j++) {
          ...
          div_game.addEventListener("mousemove", (event) => {
            if (dragging_piece) {
              ...
              dragged_piece.divcontainer.style.left = thiscamera.getx_px(dragged_piece.x) + "px";
              dragged_piece.divcontainer.style.top = thiscamera.gety_px(dragged_piece.y) + "px";
  	...
  	/*for (var i = 0; i < thispuzzle.layout[0]; i++) {
          ...
          thispuzzle.pieces[i][j].img.addEventListener("mousedown", (event) => {
            if (event.which === 1) {
              event.stopPropagation();
              mousedown_mouselocation = [parseInt(event.clientX), parseInt(event.clientY)];
              piece_mouselocation = [thispuzzle.pieces[u][v].x, thispuzzle.pieces[u][v].y];
              dragging_piece = true;
            }
          });
          thispuzzle.pieces[i][j].img.addEventListener("mousemove", (event) => {
            if (dragging_piece) {
              var mousemove_mouselocation = [parseInt(event.clientX), parseInt(event.clientY)];
              thispuzzle.pieces[u][v].x = piece_mouselocation[0] + thiscamera.getlength_cam(mousemove_mouselocation[0] - mousedown_mouselocation[0]);
              thispuzzle.pieces[u][v].y = piece_mouselocation[1] + thiscamera.getlength_cam(mousemove_mouselocation[1] - mousedown_mouselocation[1]);
              thispuzzle.pieces[u][v].divcontainer.style.left = thiscamera.getx_px(thispuzzle.pieces[u][v].x) + "px";
              thispuzzle.pieces[u][v].divcontainer.style.top = thiscamera.gety_px(thispuzzle.pieces[u][v].y) + "px";
            }
          });
          thispuzzle.pieces[i][j].img.addEventListener("mouseup", (event) => {
            if (dragging_piece && event.which === 1) {
              dragging_piece = false;
            }
          });
