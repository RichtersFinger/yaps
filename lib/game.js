...
const GRAPHICAL_ASSETS = ["img/logo.png",
                          "img/icons/addicon.png", "img/icons/backicon.png", "img/icons/gearicon.png", "img/icons/helpicon.png", "img/icons/listicon.png", "img/icons/lockicon.png", "img/icons/motif_placeholder.png", "img/icons/playicon.png", "img/icons/reloadicon.png", "img/icons/timesicon.png",
...
function setupSocketCommunication() {
  ...
  iosocket.on('updatePuzzlePartitions', ( data ) => {
    ...
    // renew shadows/highlights
    thispuzzle.renew_partition_shadows(data.partitions);
    thispuzzle.updatePiecePositions();
    thispuzzle.updatePieceScale();
  });
...
// debugging..
/*div_game.addEventListener("dblclick", (event) => {
  console.log(thiscamera.getx_cam(event.clientX), thiscamera.gety_cam(event.clientY));
});*/
        ...
        for (let j = 0; j < thispuzzle.layout[1]; j++) {
          ...
          div_game.addEventListener("mousemove", (event) => {
            ...
            if (dragging_piece && typeof(drag_timeout) === 'undefined') {
              ...
              dragged_piece.y = piece_mouselocation[1] + thiscamera.getlength_cam((mousemove_mouselocation[1] - mousedown_mouselocation[1])/thispuzzle.aspectratio); // why is the aspectratio-factor needed?
              for (const piece of dragged_piece.partition) {
